...
public class USearchBenchmark {
    ...
    public Map<BenchmarkConfig.Precision, BenchmarkResult> runBenchmarks() throws Exception {
        ...
        for (BenchmarkConfig.Precision precision : config.getPrecisions()) {
            ...
            try {
                ...
                System.out.println(String.format(
                        "✅ %s completed - Indexing: %,dms, Search: %,dms, Throughput: %,.0f QPS", precision.getName(),
    ...
    private BenchmarkResult runSingleBenchmark(BinaryVectorLoader.VectorDataset baseVectors,
        ...
        try (Index index = createIndex(usearchMetric, quantization, baseVectors.getCols(), numBaseVectors, config)) {
            ...
            return new BenchmarkResult(precision, indexingTime, searchTime, throughputQPS, metrics.recallAtK,
                    metrics.ndcgAtK, memoryUsage, numBaseVectors, baseVectors.getCols());
        }
    }
                ...
                for (int threadId = 0; threadId < numThreads; threadId++) {
                    ...
                    CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                        try {
                            ...
                            throw new RuntimeException("Search failed in thread " + finalThreadId + " (range "
                                    + startIdx + "-" + endIdx + ")", e);
                        }
                    }, customThreadPool);
    ...
    private Index createIndex(String metric, String quantization, int dimensions, int numVectors,
            BenchmarkConfig config) throws Exception {
        try {
            return new Index.Config().metric(metric).quantization(quantization).dimensions(dimensions)
                    .capacity(numVectors).build();
        } catch (Error e) {
